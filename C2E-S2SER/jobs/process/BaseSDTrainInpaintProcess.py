...
class BaseSDTrainInpaintProcess(BaseTrainProcess):
    ...
    def sample(self, step=None, is_first=False):
        ...
        self.sd.generate_images(gen_img_config_list, sampler=sample_config.sampler,tag_gen=self.tag_gen)
    ...
    def save(self, step=None):
        ...
        if self.tag_gen is not None:
            tag_enc_filename = f'{self.job.name}{step_num}_tag_gen.pt'
            tag_enc_file_path = os.path.join(self.save_root, tag_enc_filename)
        
            torch.save({
                'model_state_dict': self.tag_gen.state_dict(),
                'step': self.step_num,
                'meta': save_meta,
            }, tag_enc_file_path)
            print(f" Saved tag_gen to {tag_enc_file_path}")
    ...
    def process_general_training_batch(self, batch: 'DataLoaderBatchDTO'):
        with torch.no_grad():
            ...
            with self.timer('prepare_latents'):
                ...
                # 原始四个 block
                lat00 = latents[:, :, 0:h,   0:w]   # 上左
                lat01 = latents[:, :, 0:h,   w:W]   # 上右
                lat10 = latents[:, :, h:H,   0:w]   # 下左
                lat11 = latents[:, :, h:H,   w:W]   # 下右

                latents_swap = torch.zeros_like(latents)

                # 上行
                latents_swap[:, :, 0:h,   0:w] = lat01
                latents_swap[:, :, 0:h,   w:W] = lat00
                
                # 下行
                latents_swap[:, :, h:H,   0:w] = lat11
                latents_swap[:, :, h:H,   w:W] = lat10
            ...
            with self.timer('prepare_noise'):
                ...
                noisy_latents_swap = self.sd.add_noise(latents_swap, noise, timesteps)   
            ...
            noisy_latents_swap.requires_grad = False
            noisy_latents = noisy_latents.detach()
            noisy_latents_swap = noisy_latents_swap.detach()
            noise.requires_grad = False
            noise = noise.detach()

        return noisy_latents, noise, timesteps, conditioned_prompts, imgs,noisy_latents_swap
        ...
        if not self.is_fine_tuning:
            if self.network_config is not None:
                ...
                self.network = NetworkClass(
                    ...
                    num_experts=self.network_config.num_experts,   
                    topk=self.network_config.topk,   
                    moe_dir=os.path.join(self.get_conf('training_folder', "."),self.name),
                    **network_kwargs
                )
                ...
                extra_weights = None
                # latest_save_path = "/root/autodl-tmp/Edit-Transfer/models/base_test/base_test_000003000.safetensors"
                # print("latest_save_path,/root/autodl-tmp/Edit-Transfer/models/base_test/base_test_000003000.safetensors")
                if latest_save_path is not None:
            ...
            self.tag_gen = TAGEncoder()
            self.tag_gen.force_to(self.device_torch, dtype=torch.float32)
            params.append({
                    'params': list(self.tag_gen.parameters()),
                    'lr': self.train_config.lr
                })
            ...
            with torch.no_grad():
                ...
                if self.step_num != self.start_step:
                    if is_sample_step:
                        ...
                        self.progress_bar.unpause()
                        self.network.flush_moe_stats(step)
