...
import torch.nn.functional as F  # <- 这里
...
class ToolkitModuleMixin:
    ...
    # def get_gate(self, x):
    #     # x: [B, in_dim] or [B, C, H, W]
    #     if x.dim() > 2:
    #         x = x.mean(dim=(2, 3))  # GAP for Conv
    #     return self.gate(x)


    def _call_forward_moe(self, hidden_states,output_shape):
        ...
        for expert_idx in range(self.num_experts):
            ...
            print("current_state",current_state.shape,routing_weights[top_x_list, idx_list, None].shape)
            current_hidden_states = self.lora_up[expert_idx](self.lora_down[expert_idx](current_state)) * routing_weights[top_x_list, idx_list, None]
        ...
        # logits = self.get_gate(x)  # [B, E]
    
        # values, indices = torch.topk(logits, self.topk, dim=-1)
        # weights = torch.softmax(values, dim=-1)
        # if self.enable_moe_logging and self.training:
        #     with torch.no_grad():
        #         # indices: [B, k]
        ...
        #         eff_experts = 1.0 / (ratios ** 2).sum()

        #     # 存到 buffer（不直接写文件，避免 IO 慢）
        #         for e in range(self.num_experts):
        #             self._moe_stats_buffer["expert_id"].append(e)
        #             self._moe_stats_buffer["selection_count"].append(counts[e].item())
        #             self._moe_stats_buffer["selection_ratio"].append(ratios[e].item())
        #             self._moe_stats_buffer["gate_entropy"].append(entropy.item())
        #             self._moe_stats_buffer["effective_experts"].append(eff_experts.item())
        ...
        #     # 对应这个 expert 的权重
        #     w = weights[mask][indices[mask] == e]
            
        #     x_e = x[mask]
        #     out_e = self.lora_up[e](self.lora_down[e](x_e))
        #     out_e = out_e * self.scale
        #     out_e = out_e * w.unsqueeze(-1)
    ...
    def forward(self: Module, x, *args, **kwargs):
        ...
        # print(org_forwarded,"org_forwarded")
        if self.__class__.__name__ == "LoRAModule":
            lora_input = x.to(self.lora_down.weight.dtype)
            lora_output = self._call_forward(lora_input)
        elif self.__class__.__name__ == "LoRAMoEModule":
            lora_input = x.to(self.lora_down[0].weight.dtype)
            lora_output, router_logits = self._call_forward_moe(lora_input,org_forwarded.shape,x.shape[-1])
...
class ToolkitNetworkMixin:
    ...
    def _update_torch_multiplier(self: Network):
        ...
        if first_module.__class__.__name__ == "LoRAModule":
            device = first_module.lora_down.weight.device
            dtype = first_module.lora_down.weight.dtype
        elif first_module.__class__.__name__ == "LoRAMoEModule":
            device = first_module.lora_down[0].weight.device
            dtype = first_module.lora_down[0].weight.dtype
        with torch.no_grad():
