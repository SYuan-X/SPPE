...
data_dir = "your_results"
src_dir = "xxx/TEST/source"
edited_examplar_dir = "xxxx/TEST/surrogate"
edited_source_dir = 'xxx/TEST/source'
...
for path in path_list:
    ...
    if os.path.exists(src_path):
        src_image = load_image2(src_path)
    ...
    generated_image = load_image(path)
    ...
    sim_src_text_list.append(sim_src_text)
    sim_examplar_text_list.append(sim_test_text)
    consistency_score_list.append(1-abs(sim_src_text - sim_test_text))
    ...
    csv_writer.writerow([
        ...
        round(ssim_score, 6),
        round(psnr_score, 6),
        round(sim_src_text, 6),
        round(sim_test_text, 6),
        round(1 - abs(sim_src_text - sim_test_text), 6),
    ...
    if count==10 or count%1000==0:
        ...
        print(f"Average sim_src_keep: {average(sim_src_keep_list):.4f}")
        print(f"Average SSIM: {average(ssim_score_list):.4f}")
        print(f"Average PSNR: {average(psnr_score_list):.4f}")
        print(f"Average consistency_score (|sim_src_text - sim_test_text|): {average(consistency_score_list):.4f}")
        print(f"Average direction_sim: {average(direction_sim_list):.4f}")
        print(f"Average consistency_gt_score (|sim_src_text - sim_test_text|): {average(consistency_score_gt_list):.4f}")
...
print(f"Average SSIM: {average(ssim_score_list):.4f}")
print(f"Average PSNR: {average(psnr_score_list):.4f}")
print(f"Average consistency_score : {average(consistency_score_list):.4f}")
print(f"Average direction_sim: {average(direction_sim_list):.4f}")
print(f"Average consistency_gt_score: {average(consistency_score_gt_list):.4f}")
print(f"Average GT SSIM: {average(ssim_score_gt_list):.4f}")
print(f"Average GT PSNR: {average(psnr_score_gt_list):.4f}")
