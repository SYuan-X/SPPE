...
class SDTrainer(BaseSDTrainInpaintProcess):
    ...
    def calculate_loss(
        ...
        has_mask = batch.mask_tensor is not None and batch.mask_tensor_inv is not None
        ...
        # print("shape:",mask_multiplier.shape,noise_pred.shape)
        if not isinstance(mask_multiplier, float):
            # print("shape:",mask_multiplier.shape,noise_pred.shape)
            mask_multiplier = mask_multiplier[:,:,noise_pred.size(2)//2:,noise_pred.size(3)//2:]
    ...
    def train_single_accumulation(self, batch: DataLoaderBatchDTO):
        ...
        noisy_latents, noise, timesteps, conditioned_prompts, imgs,delta_ref,delta_test,delta_before,delta_after,gt_attn,gt_attn2 = self.process_general_training_batch(batch)
        ...
        with torch.no_grad():
            ...
            if batch.mask_tensor is not None:
                with self.timer('get_mask_multiplier'):
                    ...
                    mask_multiplier_inv = batch.mask_tensor_inv.to(self.device_torch, dtype=torch.float16).detach()
                    # scale down to the size of the latents, mask multiplier shape(bs, 1, width, height), noisy_latents shape(bs, channels, width, height)
                    mask_multiplier_inv = torch.nn.functional.interpolate(
                        mask_multiplier_inv, size=(noisy_latents.shape[2], noisy_latents.shape[3])
                    )
                    # expand to match latents
                    mask_multiplier_inv = mask_multiplier_inv.expand(-1, noisy_latents.shape[1], -1, -1)
                    mask_multiplier_inv = mask_multiplier_inv.to(self.device_torch, dtype=dtype).detach()
        ...
        with self.timer('grad_setup'):
            ...
            if self.mask_encoder is not None:
                # print(self.mask_encoder.device)
                self.mask_encoder.cuda()
                # print(self.mask_encoder.device)
                mask_encoder = self.mask_encoder
            else:
                mask_encoder = BlankNetwork()
        ...
        for noisy_latents, noise, timesteps, conditioned_prompts, imgs, adapter_images, clip_images, mask_multiplier, prompt_2 in zip(
            ...
            # print(mask_encoder.requires_grad)
            with (network):
                mask_encoder.train()
                ...
                with self.timer('encode_prompt'):
                    ...
                    if self.train_config.unload_text_encoder:
                        ...
                        with torch.set_grad_enabled(False):
                            ...
                            if isinstance(self.adapter, CustomAdapter):
                                self.adapter.is_unconditional_run = False
                            self.sd.tokenizer[0].add_special_tokens({"additional_special_tokens": ["<mask>"]})
                            mask_token_id = self.sd.tokenizer[0].convert_tokens_to_ids("<mask>")
                            # print(tokenizer.tokenize("a cat on a chair [MASK]"))

                            conditional_embeds_transfer = self.sd.encode_prompt(
                                conditioned_prompts[:len(conditioned_prompts)//2], prompt_2,
                                dropout_prob=self.train_config.prompt_dropout_prob,
                                long_prompts=self.do_long_prompts).to(
                                self.device_torch,
                                dtype=dtype)

                            # print(len(conditioned_prompts[:len(conditioned_prompts)//2][0]))

                            # print(conditional_embeds_transfer.text_embeds.size())
                            # print(delta_ref.shape)

                            conditional_embeds_restore = self.sd.encode_prompt(
                                conditioned_prompts[len(conditioned_prompts)//2:], prompt_2,
                                dropout_prob=self.train_config.prompt_dropout_prob,
                                long_prompts=self.do_long_prompts).to(
                                self.device_torch,
                        ...
                        conditional_embeds_transfer = conditional_embeds_transfer.detach()
                        conditional_embeds_restore = conditional_embeds_restore.detach()
                        if self.train_config.do_cfg:
                ...
                if batch.unconditional_latents is not None or self.do_guided_loss:
                    ...
                    def predict_and_backward(noisy_latents,timesteps,conditional_embeds,unconditional_embeds,mask_multiplier,noise):
                        with self.timer('predict_unet'):
                            if unconditional_embeds is not None:
                                unconditional_embeds = unconditional_embeds.to(self.device_torch, dtype=dtype).detach()
                            # print("noisy_latents",noisy_latents.size())
                            # network.set_transfer_grad()
                            noise_pred = self.predict_noise(
                                noisy_latents=noisy_latents.to(self.device_torch, dtype=dtype),
                                timesteps=timesteps,
                                conditional_embeds=conditional_embeds.to(self.device_torch, dtype=dtype),
                                unconditional_embeds=unconditional_embeds,
                                split_scale= self.train_config.split_scale,
                                **pred_kwargs
                            )
                            # print(noise_pred.shape)
                            # exit()
                            # decode_and_save(mask_multiplier+mask_multiplier_inv,self.sd,"total_mask","total")
                        self.after_unet_predict()

                        with self.timer('calculate_loss'):
                            noise = noise.to(self.device_torch, dtype=dtype).detach()
                            # noisy_latents_transfer, noisy_latents_restore = noisy_latents.chunk(2, dim=0)
                            # network.set_restore_grad()
                            loss_all = self.calculate_loss(
                                noise_pred=noise_pred,
                                noise=noise,
                                noisy_latents=noisy_latents,
                                timesteps=timesteps,
                                batch=batch,
                                # mask_multiplier=mask_multiplier_inv,
                                prior_pred=prior_pred,
                            )
                            loss_region = self.calculate_loss(
                                noise_pred=noise_pred,
                                noise=noise,
                                noisy_latents=noisy_latents,
                                timesteps=timesteps,
                            ...
                            loss = loss_all + loss_region
                            # /(loss_all / loss_all).detach()

                            # loss_t_all = loss_t_all+loss_t
                            # loss_r_all = loss_r_all+loss_r
                            return loss
                    # noise_pred_transfer, noise_pred_restore = noise_pred.chunk(2, dim=0)

                    noise_transfer, noise_restore = noise.chunk(2, dim=0)
                    noisy_latents_transfer, noisy_latents_restore = noisy_latents.chunk(2, dim=0)
                    timesteps_transfer, timesteps_restore = timesteps.chunk(2, dim=0)

                    mask_input = batch.mask_tensor.to(self.device_torch)
                    # scale down to the size of the latents, mask multiplier shape(bs, 1, width, height), noisy_latents shape(bs, channels, width, height)
                    mask_input = torch.nn.functional.interpolate(
                        mask_input, size=(noisy_latents.shape[2], noisy_latents.shape[3])
                    )
                    mask_input = mask_input.to(self.device_torch)
                    B, C, H, W = mask_input.shape  # 例如 B=1, C=16, H=128, W=128


                    H2, W2 = H // 2, W // 2
                    mask_input  = mask_input[:, :, H2:H, W2:W]
                    mask_input = mask_input.repeat(1,16,1,1)
                    mask_input = rearrange(
                        mask_input,
                    ...
                    mask_input.requires_grad=True
                    bs, c, h, w = delta_ref.shape
                    delta_ref = rearrange(
                        delta_ref,
                        "b c (h ph) (w pw) -> b (h w) (c ph pw)",
                        ph=2,
                    ...
                    delta_ref = delta_ref.cuda()
                    delta_ref.requires_grad=True
                    bs, c, h, w = delta_before.shape

                    delta_before = rearrange(
                        delta_before,
                        "b c (h ph) (w pw) -> b (h w) (c ph pw)",
                        ph=2,
                        pw=2
                    ...
                    delta_before = delta_before.cuda()
                    delta_before.requires_grad=True
                    
                    # network.set_restore_grad()
                    mask_encoder.set_restore_grad()
                    
                    mask_embedding = mask_encoder(mask_input,flag="mask")
                    diff_embedding = mask_encoder(delta_ref,flag="diff")
                    # print(diff_embedding.shape)
                    
                    mask_embedding = mask_embedding.to(dtype=dtype)
                    diff_embedding = diff_embedding.to(dtype=dtype)
                    conditional_embeds_restore.text_embeds = torch.cat([diff_embedding,mask_embedding,conditional_embeds_restore.text_embeds],dim=1)

                    loss_restore = predict_and_backward(noisy_latents_restore,timesteps_restore,conditional_embeds_restore,unconditional_embeds,mask_multiplier,noise_restore)
                  
                    if torch.isnan(loss_restore):
                        print("loss_all is nan")
                        loss_restore = torch.zeros_like(loss_restore).requires_grad_(True)

                    with self.timer('backward'):
                        loss_restore = loss_restore * loss_multiplier.mean() 
                        # + loss_mask
                        if not self.do_grad_scale:
                            loss_restore.backward()
                        else:
                            self.scaler.scale(loss_restore).backward()


                    mask_encoder.set_transfer_grad()
                    
                    ref_before_latent  = noisy_latents[:, :, 0:H2, 0:W2] 
                    ref_after_latent   = noisy_latents[:, :, 0:H2, W2:W]
                    mask_embedding = mask_encoder(mask_input,flag="mask")
                    diff_embedding = mask_encoder(delta_ref,flag="diff")
                    mask_embedding = mask_embedding.to(dtype=dtype)
                    diff_embedding = diff_embedding.to(dtype=dtype)
                    ...
                    conditional_embeds_transfer.text_embeds = torch.cat([diff_embedding,mask_embedding,conditional_embeds_transfer.text_embeds],dim=1)
                    loss_transfer = predict_and_backward(noisy_latents_transfer,timesteps_transfer,conditional_embeds_transfer,unconditional_embeds,mask_multiplier_inv,noise_transfer)
                    if torch.isnan(loss_transfer):
                        print("loss_all is nan")
                        loss_transfer = torch.zeros_like(loss_transfer).requires_grad_(True)

                    with self.timer('backward'):
                        loss_transfer = loss_transfer * loss_multiplier.mean()
                        #  + loss_mask
                        
                        if not self.do_grad_scale:
                            loss_transfer.backward()
                        else:
                            self.scaler.scale(loss_transfer).backward()
                    




        loss = loss_transfer.detach()+loss_restore.detach()
        # +loss_restore.detach() 
        # print(loss_r_all,loss_t_all,file = "losslog.log")
        with open("losslog_multiembed_share.log", "a") as f:
            print(loss_transfer.detach(),loss_restore.detach(), file=f)
            # print(loss_transfer.detach(), loss_restore.detach(), file=f)
        ...
        if not self.is_grad_accumulation_step:
            ...
            if self.train_config.optimizer != 'adafactor':
                if self.do_grad_scale:
                    self.scaler.unscale_(self.optimizer_transfer)
                    self.scaler.unscale_(self.optimizer_restore)
                    self.scaler.unscale_(self.optimizer_mask)
                if isinstance(self.params_transfer[0], dict):
                    for i in range(len(self.params_transfer)):
                        torch.nn.utils.clip_grad_norm_(self.params_transfer[i]['params'], self.train_config.max_grad_norm)
                else:
                    torch.nn.utils.clip_grad_norm_(self.params_transfer, self.train_config.max_grad_norm)
                if isinstance(self.params_restore[0], dict):
                    for i in range(len(self.params_restore)):
                        torch.nn.utils.clip_grad_norm_(self.params_restore[i]['params'], self.train_config.max_grad_norm)
                else:
                    torch.nn.utils.clip_grad_norm_(self.params_restore, self.train_config.max_grad_norm)
            ...
            with self.timer('optimizer_step'):
                ...
                if not self.do_grad_scale:
                    ...
                    self.scaler.step(self.optimizer_transfer)
                    self.scaler.step(self.optimizer_restore)
                    self.scaler.step(self.optimizer_mask)
                    self.scaler.update()

                self.optimizer_transfer.zero_grad(set_to_none=True)
                self.optimizer_restore.zero_grad(set_to_none=True)
                self.optimizer_mask.zero_grad(set_to_none=True)
        else:
    ...
    def hook_train_loop(self, batch: Union[DataLoaderBatchDTO, List[DataLoaderBatchDTO]]):
        ...
        with self.timer('scheduler_step'):
            self.lr_scheduler_transfer.step()
            self.lr_scheduler_restore.step()
