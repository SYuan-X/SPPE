...
from toolkit.stable_diffusion_model_inpaint import StableDiffusion
...
from utils import extract_edit_instruction,extract_edit_instruction_and_object,save_mask_heatmaps_batch
...
class BaseSDTrainInpaintProcess(BaseTrainProcess):
    ...
    def sample(self, step=None, is_first=False):
        ...
        for i in range(len(sample_config.prompts)):
            ...
            # transformation_txt = extract_edit_instruction(prompt)
            # transfer_prompt = f"This is a four-panel image grid. [TOP-RIGHT] is the result of applying an editing transformation {transformation_txt} to [TOP-LEFT]. [BOTTOM-RIGHT] is the result of apply the same transformation to [BOTTOM-LEFT]"
            # restore_prompt = f"This is a four-panel image grid. [BOTTOM-RIGHT] restore the masked region based on the content of [BOTTOM-LEFT] and the transformations shown in  of [TOP_RIGHT]"
                            
            transformation_txt,object_txt = extract_edit_instruction_and_object(prompt)
            # print(transformation_txt)
            # if transformation_txt:
            transfer_prompt = f"This four-panel image grid shows the result of applying the transformation '{transformation_txt}' from [TOP-LEFT] to [TOP-RIGHT], and then from [BOTTOM-LEFT] to [BOTTOM-RIGHT]."
            ...
            restore_prompt = transfer_prompt
            ...
            # print("sample_config")
            # print(sample_config)
            # print("image = sample_config.image")
            # print(sample_config.image)
            # print(type(sample_config.image))
        ...
        self.sd.generate_images(gen_img_config_list, sampler=sample_config.sampler,mask_encoder=self.mask_encoder)
    ...
    def save(self, step=None):
            ...
            if self.decorator is not None:
                ...
                for key, value in decorator_state_dict.items():
                    if isinstance(value, torch.Tensor):
                        decorator_state_dict[key] = value.clone().to('cpu',
                                                                     dtype=get_torch_dtype(self.save_config.dtype))
                save_file(
        ...
        # save optimizer
        if self.optimizer_transfer is not None:
            try:
                filename = f'optimizer_t.pt'
                file_path = os.path.join(self.save_root, filename)
                state_dict = self.optimizer_transfer.state_dict()
                torch.save(state_dict, file_path)
            except Exception as e:
                print(e)
                print("Could not save optimizer")
        
        if self.optimizer_restore is not None:
            try:
                filename = f'optimizer_r.pt'
                file_path = os.path.join(self.save_root, filename)
                state_dict = self.optimizer_restore.state_dict()
                torch.save(state_dict, file_path)
            except Exception as e:
                print(e)
                print("Could not save optimizer")
        ...
        if self.optimizer_mask is not None:
            try:
                filename = f'optimizer_r.pt'
                file_path = os.path.join(self.save_root, filename)
                state_dict = self.optimizer_mask.state_dict()
                torch.save(state_dict, file_path)
            except Exception as e:
                print(e)
                print("Could not save optimizer")
        ...
        if self.mask_encoder is not None:
            mask_enc_filename = f'{self.job.name}{step_num}_mask_encoder.pt'
            mask_enc_file_path = os.path.join(self.save_root, mask_enc_filename)
        
            torch.save({
                'model_state_dict': self.mask_encoder.state_dict(),
                'step': self.step_num,
                'meta': save_meta,
            }, mask_enc_file_path)
            print(f" Saved mask_encoder to {mask_enc_file_path}")
    ...
    def load_training_state_from_metadata(self, path):
        ...
        if meta is not None and 'training_info' in meta and 'step' in meta[
            'training_info'] and self.train_config.start_step is None:
    ...
    def apply_snr(self, seperated_loss, timesteps):
        if self.train_config.learnable_snr_gos:
            ...
            # add snr_gamma
            seperated_loss = apply_snr_weight(seperated_loss, timesteps, self.sd.noise_scheduler,
    ...
    def process_general_training_batch(self, batch: 'DataLoaderBatchDTO'):
        with torch.no_grad():
            with self.timer('prepare_prompt'):
                ...
                if self.train_config.transfer_and_restore:
                    transfer_prompts = []
                    restore_prompts = []
                    for prompt in conditioned_prompts:
                        ...
                        # assert not transformation_txt==None
                        if transformation_txt:
                            transfer_prompt = f"This four-panel image grid shows the result of applying the transformation '{transformation_txt}' from [TOP-LEFT] to [TOP-RIGHT], and then from [BOTTOM-LEFT] to [BOTTOM-RIGHT]."
                            # restore_prompt = f"In this four-panel image grid, [BOTTOM-RIGHT] restores the '{object_txt}' region based on [BOTTOM-LEFT] while preserving the transformation shown in [TOP-RIGHT]."
                            restore_prompt = transfer_prompt
                            transfer_prompts.append(transfer_prompt)
                            restore_prompts.append(restore_prompt)
                        else:
                            transfer_prompts.append('')
                            restore_prompts.append('')
                    conditioned_prompts = transfer_prompts+restore_prompts
            ...
            test_before_latent = noisy_latents[:, :, H2:H, 0:W2]  # 左下
            test_after_latent  = noisy_latents[:, :, H2:H, W2:W]  # 右下
            # print("batch.mask_tensor",batch.mask_tensor.shape)
            mask = batch.mask_tensor[:, :, batch.mask_tensor.size(2)//2:, batch.mask_tensor.size(3)//2:] 
            # print(mask.shape)
            mask=mask.to(ref_before_latent.device)
            mask = F.interpolate(mask, size=(64, 64), mode='bilinear', align_corners=False)  # [B, 1, h_lat, w_lat]
            
            delta_ref = (ref_after_latent - ref_before_latent).pow(2)
            # .mean(dim=1, keepdim=True)  
            # delta_ref = (delta_ref - delta_ref.min()) / (delta_ref.max() - delta_ref.min() + 1e-5) 
            delta_test = (test_after_latent - test_before_latent).pow(2)
            delta_before = (test_before_latent - ref_before_latent).pow(2)
            delta_after = (test_after_latent - ref_after_latent).pow(2)

            gt_left = torch.cat([delta_before, delta_before], dim=2)   # 上下拼 -> [B, H*2, W]
            gt_right = torch.cat([delta_after, delta_after], dim=2)    # 上下拼 -> [B, H*2, W]

            gt_mask = torch.cat([gt_left, gt_right], dim=3)  # 左右拼 -> [B, H*2, W*2]

            gt_left2 = torch.cat([delta_ref, delta_ref], dim=3)   # 上下拼 -> [B, H*2, W]
            gt_right2 = torch.cat([delta_test, delta_test], dim=3)    # 上下拼 -> [B, H*2, W]

            gt_mask2 = torch.cat([gt_left2, gt_right2], dim=2)  # 左右拼 -> [B, H*2, W*2]
            # gt_mask_resized = F.interpolate(gt_mask, size=(H, W), mode='bilinear', align_corners=False)  # [B, 1, h_lat, w_lat]
            ...
            if do_double:
                ...
                # print("do_double_after",noisy_latents.shape)
                ...
                # batch.mask_tensor_inv = double_up_tensor(batch.mask_tensor_inv)
            ...
            from utils import save_mask_heatmaps_batch
            
            
            # save_mask_heatmaps_batch(delta_ref,prefix="delta_ref.jpg")
            # save_mask_heatmaps_batch(delta_test,prefix="delta_test.jpg")
            # save_mask_heatmaps_batch(delta_before,prefix="delta_before.jpg")
            # save_mask_heatmaps_batch(delta_after,prefix="delta_after.jpg")
            # save_mask_heatmaps_batch(gt_mask,prefix="gt_mask.jpg")
            # save_mask_heatmaps_batch(ref_before_latent.mean(dim=1,keepdim=True),prefix="ref_before.jpg")
            # save_mask_heatmaps_batch(ref_after_latent.mean(dim=1,keepdim=True),prefix="ref_after.jpg")
            # save_mask_heatmaps_batch(test_before_latent.mean(dim=1,keepdim=True),prefix="test_before.jpg")
            # save_mask_heatmaps_batch(test_after_latent.mean(dim=1,keepdim=True),prefix="test_after.jpg")


            # print(noisy_latents.shape)
            # exit()

        return noisy_latents, noise, timesteps, conditioned_prompts, imgs,delta_ref,delta_test,delta_before,delta_after,gt_mask,gt_mask2
    ...
    def run(self):
        ...
        if not self.is_fine_tuning:
            if self.network_config is not None:
                ...
                # params += params_net
                params_t += params_net[0]
                params_r += params_net[1]
                # exit()
            ...
            params_t = self.load_additional_training_modules(params_t)
            params_r = self.load_additional_training_modules(params_r)


            self.mask_encoder = MaskEncoder()
            self.mask_encoder.force_to(self.device_torch, dtype=torch.float32)

            # mask_embed = mask_encoder(delta_test)  # [B, 64]
        ...
        params_r = self.hook_add_extra_train_params(params_r)
        params_t = self.hook_add_extra_train_params(params_t)
        # self.params = params
        self.params_transfer = params_t
        self.params_restore = params_r
        if self.mask_encoder is not None:
            self.params_mask = self.mask_encoder.parameters()
        # self.params = []
        ...
        optimizer_transfer = get_optimizer(self.params_transfer, optimizer_type, learning_rate=self.train_config.lr,
                                  optimizer_params=self.train_config.optimizer_params)
        optimizer_restore = get_optimizer(self.params_restore, optimizer_type, learning_rate=self.train_config.lr,
                                  optimizer_params=self.train_config.optimizer_params)
        optimizer_mask = get_optimizer(self.params_mask, optimizer_type, learning_rate=self.train_config.lr,
                                  optimizer_params=self.train_config.optimizer_params)
        # self.optimizer = optimizer
        self.optimizer_transfer = optimizer_transfer
        self.optimizer_restore = optimizer_restore
        ...
        if self.train_config.do_paramiter_swapping:
            ...
            self.optimizer_transfer.enable_paramiter_swapping(self.train_config.paramiter_swapping_factor)
            self.optimizer_restore.enable_paramiter_swapping(self.train_config.paramiter_swapping_factor)
            self.optimizer_mask.enable_paramiter_swapping(self.train_config.paramiter_swapping_factor)

        # check if it exists
        # optimizer_state_filename = f'optimizer.pt'
        # optimizer_state_file_path = os.path.join(self.save_root, optimizer_state_filename)
        optimizer_state_filename_t = f'optimizer_t.pt'
        optimizer_state_file_path_t = os.path.join(self.save_root, optimizer_state_filename_t)
        optimizer_state_filename_r = f'optimizer_r.pt'
        optimizer_state_file_path_r = os.path.join(self.save_root, optimizer_state_filename_r)
        if os.path.exists(optimizer_state_filename_t) and os.path.exists(optimizer_state_filename_r):
            # try to load
            ...
            previous_lrs_r = []
            previous_lrs_m = []
            for group in optimizer_transfer.param_groups:
                previous_lrs_t.append(group['lr'])
            for group in optimizer_restore.param_groups:
                previous_lrs_r.append(group['lr'])
            for group in optimizer_mask.param_groups:
                previous_lrs_m.append(group['lr'])

            try:
                print(f"Loading optimizer state from {optimizer_state_file_path_t} and {optimizer_state_file_path_r}")
                optimizer_state_dict_t = torch.load(optimizer_state_file_path_t, weights_only=True)
                optimizer_transfer.load_state_dict(optimizer_state_dict_t)
                optimizer_state_dict_r = torch.load(optimizer_state_file_path_r, weights_only=True)
                optimizer_restore.load_state_dict(optimizer_state_dict_r)
                optimizer_state_dict_m = torch.load(optimizer_state_file_path_m, weights_only=True)
                optimizer_mask.load_state_dict(optimizer_state_dict_m)
                del optimizer_state_dict_t,optimizer_state_dict_r
                flush()
            except Exception as e:
                print(f"Failed to load optimizer state from {optimizer_state_file_path_t} and  {optimizer_state_file_path_r}")
                print(e)

            # update the optimizer LR from the params
            print(f"Updating optimizer LR from params")
            if len(previous_lrs_t) > 0:
                for i, group in enumerate(optimizer_transfer.param_groups):
                    group['lr'] = previous_lrs_t[i]
                    group['initial_lr'] = previous_lrs_t[i]
            if len(previous_lrs_r) > 0:
                for i, group in enumerate(optimizer_restore.param_groups):
                    group['lr'] = previous_lrs_r[i]
                    group['initial_lr'] = previous_lrs_r[i]
            if len(previous_lrs_m) > 0:
                for i, group in enumerate(optimizer_mask.param_groups):
                    group['lr'] = previous_lrs_m[i]
                    group['initial_lr'] = previous_lrs_m[i]
        ...
        lr_scheduler_params_t = self.train_config.lr_scheduler_params
        lr_scheduler_params_r = self.train_config.lr_scheduler_params
        lr_scheduler_params_m = self.train_config.lr_scheduler_params

        # make sure it had bare minimum
        if 'max_iterations' not in lr_scheduler_params_t:
            lr_scheduler_params_t['total_iters'] = self.train_config.steps
        if 'max_iterations' not in lr_scheduler_params_r:
            lr_scheduler_params_r['total_iters'] = self.train_config.steps
        if 'max_iterations' not in lr_scheduler_params_m:
            lr_scheduler_params_m['total_iters'] = self.train_config.steps

        lr_scheduler_t = get_lr_scheduler(
            self.train_config.lr_scheduler,
            optimizer_transfer,
            **lr_scheduler_params_t
        )
        lr_scheduler_r = get_lr_scheduler(
            self.train_config.lr_scheduler,
            optimizer_restore,
            **lr_scheduler_params_r
        )
        lr_scheduler_m = get_lr_scheduler(
            self.train_config.lr_scheduler,
            optimizer_mask,
            **lr_scheduler_params_m
        )
        # self.lr_scheduler = lr_scheduler
        self.lr_scheduler_transfer = lr_scheduler_t
        self.lr_scheduler_restore = lr_scheduler_r
        self.lr_scheduler_mask = lr_scheduler_m
        ...
        self.lr_scheduler_transfer.step(self.step_num)
        self.lr_scheduler_restore.step(self.step_num)
        self.lr_scheduler_mask.step(self.step_num)
        ...
        for step in range(start_step_num, self.train_config.steps):
            if self.train_config.do_paramiter_swapping:
                ...
                self.optimizer_transfer.swap_paramiters()
                self.optimizer_restore.swap_paramiters()
                self.optimizer_mask.swap_paramiters()
            ...
            with torch.no_grad():
                ...
                if hasattr(optimizer_transfer, 'get_avg_learning_rate') and hasattr(optimizer_restore, 'get_avg_learning_rate')and hasattr(optimizer_mask, 'get_avg_learning_rate'):
                    # learning_rate = optimizer.get_avg_learning_rate()
                    learning_rate_t = optimizer_transfer.get_avg_learning_rate()
                    learning_rate_r = optimizer_restore.get_avg_learning_rate()
                    learning_rate_m = optimizer_mask.get_avg_learning_rate()
                elif hasattr(optimizer_transfer, 'get_learning_rates') and hasattr(optimizer_restore, 'get_learning_rates')and hasattr(optimizer_mask, 'get_learning_rates'):
                    # learning_rate = optimizer.get_learning_rates()[0]
                    learning_rate_t = optimizer_transfer.get_learning_rates()[0]
                    learning_rate_r = optimizer_restore.get_learning_rates()[0]
                    learning_rate_m = optimizer_mask.get_learning_rates()[0]
                    ...
                    learning_rate_t = optimizer_transfer.param_groups[0]['lr']
                    learning_rate_r = optimizer_restore.param_groups[0]['lr']
                    learning_rate_m = optimizer_mask.param_groups[0]['lr']

                # prog_bar_string = f"lr: {learning_rate:.1e}"
                prog_bar_string = f"lr(transfer): {learning_rate_t:.1e},lr(restore): {learning_rate_t:.1e}"
                    ...
                    if self.logging_config.log_every and self.step_num % self.logging_config.log_every == 0:
                        ...
                        with self.timer('log_to_tensorboard'):
                            ...
                            if self.writer is not None:
                                ...
                                self.writer.add_scalar(f"lr(transfer)", learning_rate_t, self.step_num)
                                self.writer.add_scalar(f"lr(restore)", learning_rate_r, self.step_num)
                            self.progress_bar.unpause()

                        # log to logger
                        self.logger.log({
                            'learning_rate(t)': learning_rate_t,
                            'learning_rate(r)': learning_rate_r,
                        })
                        for key, value in loss_dict.items():
                            self.logger.log({
                                f'loss/{key}': value,
