...
class TrainConfig:
    def __init__(self, **kwargs):
        self.transfer_and_restore = kwargs.get('transfer_and_restore', False)
        self.split_scale = kwargs.get('split_scale', False)
...
class ModelConfig:
    def __init__(self, **kwargs):
        self.transfer_and_restore: str = kwargs.get('transfer_and_restore', False)
...
class GenerateImageConfig:
    def __init__(
        ...
        self.prompt_transfer: str = prompt_transfer
        self.prompt_restore: str = prompt_restore
    ...
    def _get_path_no_ext(self, prefix, count: int = 0, max_count=0):
        # zero pad count
    ...
    def get_image_path(self,prefix, count: int = 0, max_count=0):
        filename = self._get_path_no_ext(prefix,count, max_count)
        ext = self.output_ext
    ...
    def save_image(self, image,prefix, count: int = 0, max_count=0):
        # make parent dirs
        os.makedirs(self.output_folder, exist_ok=True)
        self.set_gen_time()
        # TODO save image gen header info for A1111 and us, our seeds probably wont match
        image.save(self.get_image_path(prefix,count, max_count))
        # do prompt file
