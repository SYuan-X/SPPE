    ...
    def __init__(self, **kwargs):
        try:
            ...
            if any([x.mask_tensor is not None for x in self.file_items]):
                ...
                mask_tensors_inv = []
                for x in self.file_items:
                    if x.mask_tensor_inv is None:
                        mask_tensors_inv.append(torch.zeros_like(base_mask_tensor))
                    else:
                        mask_tensors_inv.append(x.mask_tensor_inv)
                self.mask_tensor_inv = torch.cat([x.unsqueeze(0) for x in mask_tensors_inv])
