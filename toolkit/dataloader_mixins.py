...
class MaskFileItemDTOMixin:
    ...
    def load_mask_image(self: 'FileItemDTO'):
        ...
        top = int(h * (1 - ratio))
        inv_img = img.copy()
        region = inv_img.crop((left, top, w, h))
        inverted_region = ImageOps.invert(region)

        # 粘贴回去
        inv_img.paste(inverted_region, (left, top))
        ...
        if self.flip_x:
            # do a flip
            img = img.transpose(Image.FLIP_LEFT_RIGHT)
            inv_img = inv_img.transpose(Image.FLIP_LEFT_RIGHT)
        if self.flip_y:
            # do a flip
            img = img.transpose(Image.FLIP_TOP_BOTTOM)
            inv_img = inv_img.transpose(Image.FLIP_TOP_BOTTOM)
        ...
        inv_img = inv_img.filter(ImageFilter.GaussianBlur(radius=blur_radius))
        ...
        if self.dataset_config.buckets:
            ...
            inv_img = inv_img.resize((self.scale_to_width, self.scale_to_height), Image.BICUBIC)
            # img = transforms.CenterCrop((self.crop_height, self.crop_width))(img)
            # crop
            inv_img = inv_img.crop((
                self.crop_x,
                self.crop_y,
                self.crop_x + self.crop_width,
                self.crop_y + self.crop_height
        ...
        if self.aug_replay_spatial_transforms:
            ...
            self.mask_tensor_inv = self.augment_spatial_control(inv_img, transform=transform)
        else:
            self.mask_tensor = transform(img)
            self.mask_tensor_inv = transform(inv_img)
        self.mask_tensor = value_map(self.mask_tensor, 0, 1.0, self.mask_min_value, 1.0)
        self.mask_tensor_inv = value_map(self.mask_tensor_inv, 0, 1.0, self.mask_min_value, 1.0)
        # convert to grayscale

    def cleanup_mask(self: 'FileItemDTO'):
        self.mask_tensor = None
        self.mask_tensor_inv = None
            ...
            for file_item in tqdm(self.file_list, desc=f'Caching latents{" to disk" if to_disk else ""}'):
                ...
                if os.path.exists(latent_path):
                    ...
                    try:
                        ...
                        # 切分成四个部分
                        parts = []
                        parts.append(imgs[..., :h_half, :w_half])  # 左上
                        parts.append(imgs[..., :h_half, w_half:])  # 右上
                        parts.append(imgs[..., h_half:, :w_half])  # 左下
                        parts.append(imgs[..., h_half:, w_half:])  # 右下
