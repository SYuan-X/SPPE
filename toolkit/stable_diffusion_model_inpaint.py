...
class StableDiffusion:
        ...
        with network:
            with torch.no_grad():
                ...
                for i in tqdm(range(len(image_configs)), desc=f"Generating Images", leave=False):
                    ...
                    conditional_embeds = self.encode_prompt([gen_config.prompt_transfer,gen_config.prompt_restore], gen_config.prompt_2, force_all=True)
                    ...
                    if self.is_xl:
                        ...
                        if self.model_config.use_flux_cfg:
                            ...
                            #     **extra
                            # ).images[0]
                            mask_sensitive = Image.open(gen_config.image.replace("edit_transfer_test_new", "MASK_TEST")).convert("L")
                            w, h = mask_sensitive.size
                            crop_w, crop_h = w // 2, h // 2
                            region = mask_sensitive.crop((w - crop_w, h - crop_h, w, h))
                            inverted_region = ImageOps.invert(region)
                            mask_sensitive_inv = mask_sensitive.copy()
                            mask_sensitive_inv.paste(inverted_region, (w - crop_w, h - crop_h))

                            # print(gen_config.latents.shape)
                            

                            # images_t,images_r = pipeline(
                            img = pipeline(
                                image=Image.open(
                                    gen_config.image).convert('RGB'),
                                mask_image =Image.open("mask.png"),
                                ...
                                prompt_transfer=gen_config.prompt_transfer,
                                prompt_restore=gen_config.prompt_restore,
                                # prompt_transfer=gen_config.prompt_transfer,
                                # prompt_embeds=conditional_embeds.text_embeds,
                                # pooled_prompt_embeds=conditional_embeds.pooled_embeds,
                                # image=img,
                            ...
                            # img_t = images_t.images[0]
                            # # print(img_t.)
                            # img_r = images_r.images[0]
                    elif self.is_pixart:
                    ...
                    # gen_config.save_image(img_t,"transfer_", i)
                    # gen_config.save_image(img_r,"restore_", i)
                    # gen_config.log_image(img_t, i)
                    gen_config.save_image(img,"mask_", i)
    ...
    def predict_noise(
            ...
            # mask_multiplier: torch.Tensor= None,
            # mask_multiplier_inv: torch.Tensor= None,
            split_scale=1,
            **kwargs,
    ):
        ...
        if self.is_xl:
                ...
                if self.is_flux:
                    with torch.no_grad():
                        ...
                        if self.unet.config.guidance_embeds:
                            ...
                            elif self.model_config.transfer_and_restore:
                                guidance = torch.tensor([guidance_embedding_scale], device=self.device_torch)
            ...
            # if self.model_config.transfer_and_restore:
            #     noise_pred_transfer, noise_pred_restore = noise_pred.chunk(2, dim=0)

            #     conditional_pred = noise_pred_transfer
            #     noise_pred = mask_multiplier_inv*noise_pred_transfer + mask_multiplier*noise_pred_restore
